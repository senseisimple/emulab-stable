 ...
 * Date: 28 Mar 2002
 * CVS: $Id: opt.c,v 1.1 2004-12-12 23:36:33 johnsond Exp $
 * Notes:
 * - This will leak mem due to the strdup's; should fix this sometime.
 * - Currently inserts extra space in front of some things when saved.
 **************************************************************************/
...
// Add an item
void opt_add_item(const char *section, const char *key,
                  const char *value, const char *comment, int save);

// Set the value of an existing item
void opt_set_item(const char *section, const char *key,
                  const char *value, const char *comment, int save);


// The one and only instance of the options
static opt_t *opt;
...
int opt_init(int argc, char **argv, const char *filename)
{
  ...
  for (i = 1; i < argc; i++)
  {
    if (strncmp(argv[i], "--", 2) == 0 && i + 1 < argc)
    {
      ...
      value = argv[i + 1];
      opt_add_item(section, key, value, NULL, 0);
      free(tmp);
    }
  }
...
int opt_load( const char *filename)
{
  ...
  for (i = 0; 1; i++)
  {
    ...
    // Handle blank lines
    if (tokens[0] == NULL)
      opt_add_item(NULL, NULL, NULL, line, 1);
    ...
    else if (tokens[0] && tokens[1] && tokens[2])
    {
      ...
      opt_add_item(section, key, value, NULL, 1);
    }
    else
    {
...
{
  if (opt_get_string(section, key, NULL) == NULL)
    opt_add_item(section, key, value, NULL, 1);
  opt_set_item(section, key, value, NULL, 1);
}
...
{
  const char *value;
  value = opt_get_string(section, key, NULL);
  if (value != NULL)
    return atoi(value);
  else
    return defvalue;
...
// Write an integer
void opt_set_int( const char *section, const char *key,
                   int value)
{
  char svalue[64];
  snprintf(svalue, sizeof(svalue), "%d", value);
  opt_set_string(section, key, svalue);
}

// Read in two integers
// Returns 0 if successful
int opt_get_int2( const char *section, const char *key,
                   int *p1, int *p2)
{
  const char *value;
...
// Write two integers
void opt_set_int2( const char *section, const char *key,
                    int p1, int p2)
{
  char svalue[64];
...
// Read a double
double opt_get_double( const char *section, const char *key,
                        double defvalue)
{
  const char *value;
  value = opt_get_string(section, key, NULL);
...
// Read in two doubles
// Returns 0 if successful
int opt_get_double2( const char *section, const char *key,
                      double *p1, double *p2)
{
  const char *value;
