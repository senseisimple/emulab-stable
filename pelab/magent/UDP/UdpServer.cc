...
unsigned long long getPcapTimeMilli(const struct timeval *tp)
{
...
//	gettimeofday(&timeVal, NULL);
	...
	if(packetType == '0')
	{
		...
		if(packetSeqNum > curSeqNum)
		{
			...
			milliSec = getPcapTimeMilli(timeStamp);
...
//	gettimeofday(&timeVal, NULL);
//	std::cout << "Time After = "<<getPcapTimeMilli(&timeVal)<<std::endl;
}
...
//	std::cout << "Time before = "<<getPcapTimeMilli(&timeVal)<<std::endl;
   ...
   //     printf("Data being received = %c, %u\n", *(unsigned char *)(dataPtr), *(unsigned int *)(dataPtr + 1));
		...
		milliSec = getPcapTimeMilli(&pcap_info->ts);
		if(lastAckTime == 0)
			lastAckTime = milliSec;
		else
		{
...
void init_pcap()
{
        ...
        sprintf(filter," udp and dst 10.1.1.2 and dst port 1500");
...
int main(int argc, char *argv[]) 
{
	...
	messageBuf.msg_name = &cliAddr;
	messageBuf.msg_namelen = sizeof(struct sockaddr_in);

	messageBuf.msg_control = (void *)malloc(64);
	messageBuf.msg_controllen = 64;
	...
	while(1) 
	{
		...
		/*
		n = recvmsg(sd, &messageBuf, flags);

		chdr = CMSG_FIRSTHDR(&messageBuf);

		timeStamp = *(struct timeval *)CMSG_DATA(chdr);

		handleUDPMsg(&cliAddr, msg, n, &timeStamp, outFile);
		*/
