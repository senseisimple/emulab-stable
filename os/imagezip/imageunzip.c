...
/*      extern char     build_info[]; */
        ...
        nothreads = 1;
#endif
        while ((ch = getopt(argc, argv, "vdhs:zp:oOnFD:W:Cr:ck:")) != -1)
                switch(ch) {
#ifdef FAKEFRISBEE
                case 'F':
                        dofrisbee++;
...
inflate_subblock(const char *chunkbufp)
{
        ...
        case COMPRESSED_V4:
                if (use_checksum) {
                        if (! verify_checksum((blockhdr_t *)blockhdr,
                ...
                if (has_id) {
                  if (memcmp(imageid, blockhdr->imageid, UUID_LENGTH))
                  {
                    // Not the same
                    char uuidstr[UUID_LENGTH*2 + 1];
                    fprintf(stderr, "Image ID fails to match:\n");
                    mem_to_hex(uuidstr, imageid, UUID_LENGTH);
                    fprintf(stderr, "  Stored: 0x%s\n", uuidstr);
                    mem_to_hex(uuidstr, blockhdr->imageid, UUID_LENGTH);
                    fprintf(stderr, "  Chunk: 0x%d\n", uuidstr);
                    exit(0);
                  }
                } else {
                  has_id = 1;
