...
#ifdef WITH_CRYPTO
static void output_public_key(RSA * key);
RSA * signature_key = NULL;
#endif
...
/*      extern char build_info[]; */
        ...
        while ((ch = getopt(argc, argv, "vlbnNdihrs:c:z:oI:1F:DR:S:XC:H:Me:k:u:")) != -1)
                switch(ch) {
                case 'v':
                        version++;
                        break;
                ...
                case 'u':
                  // UUID for image id.
                  if (!hex_to_mem(imageid, optarg, UUID_LENGTH)) {
                    usage();
                  }
                  break;
                case 'h':
...
#ifdef WITH_CRYPTO
        signature_key = RSA_generate_key(CHECKSUM_LEN_MAX*8, 17, NULL, NULL);
        output_public_key(signature_key);
        /*
...
//void authenticate(blockhdr_t *hdr);
#endif
...
static void
output_public_key(RSA * key) {
  FILE * file = fopen("public.sig", "w");
...
checksum_finish(blockhdr_t *hdr) {
         ...
         */
#ifdef SIGN_CHECKSUM
        unsigned char checksum[CHECKSUM_LEN_MAX - 11];
        int result = 0;

        SHA1_Final(checksum, &sha_ctx);

        result = RSA_private_encrypt(CHECKSUM_LEN_MAX - 11, checksum, hdr->checksum,
...
void authenticate(blockhdr_t *hdr) {
  int bytecount = 0;
  if (EVP_PKEY_size(authkey) > AUTHENTICATION_SIGNATURE_LENGTH)
  {
    fprintf(stderr, "Authorization signature size is too large: %d", EVP_PKEY_size(authkey));
    exit(1);
  }
  ...
  hdr->authtype = AUTHENTICATION_RSA;
}
*/
/*
