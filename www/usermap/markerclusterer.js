  ...
  this.redraw_ = function (isForce) {
    ...
    if (zoom_ >= mz) {
      ...
      if (clusterMarker_ === null) {
        ...
        for (var i = 0; i < markers_.length; i++) {
            var city = new Object();
            city.name = markers_[i].marker.name;
            city.count = markers_[i].marker.count;
            cities.push(city);
            }
        cities = cities.sort(function(a,b) { return a.count < b.count; });
        clusterMarker_ = new ClusterMarker_(center_, this.getPopulation(), markerClusterer_.getStyles_(), markerClusterer_.getGridSize_(),cities);
 ...
 */
function ClusterMarker_(latlng, count, styles, padding, cities) {
  var index = 0;
  var dv = count;
  while (dv !== 0) {
    dv = parseInt(dv / 4, 10);
...
ClusterMarker_.prototype.initialize = function (map) {
  ...
  for (var i = 0; i < cities.length; i++) {
    if (i == 10 && cities.length >= 10) {
        var remaining = cities.length - i;
        infohtml += "<li><i>... and " + remaining.toString() + " others</i></li>";
        break;
    } else {
        infohtml += "<li>" + cities[i].name + " <i>(" + cities[i].count.toString() + ")</i></li>";
    }
  }
  infohtml += "</ul>";
...
ClusterMarker_.prototype.copy = function () {
  return new ClusterMarker_(this.latlng_, this.index_, this.text_, this.styles_, this.padding_,this.cities_);
};
