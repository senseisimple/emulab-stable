#!/usr/bin/perl -w
use lib (split(/:/, $ENV{GITPERLLIB} || "/usr/share/perl/5.10.0"));
...
if ($opt_W) {
    ...
    if (!`GIT_DIR= git-apply $context --summary --numstat --apply <.cvsexportcommit.diff`) {
	    print "Failed to apply patch; " .
	                 "checking to see if patch was already applied...\n"; 
	    if (`GIT_DIR= git-apply $context -R --summary --numstat --check <.cvsexportcommit.diff 2>/dev/null`) {
